...
export const WISSEN: Record<string, SeitenWissen> = {
  ...
  '/dashboard/anlagen': {
    ...
    schritte: [
      ...
      '„💾 Anlagegut speichern“. Wird etwas verkauft oder ausgemustert, das Anlagegut über „Bearbeiten“ öffnen, den Status ändern und das „Abgangsdatum“ eintragen (Pflicht). Die AfA läuft dann nur bis zum Abgangsmonat, der Restbuchwert zählt im Abgangsjahr als Ausgabe',
  ...
  '/dashboard/euer': {
    ...
    schritte: [
      ...
      'Die Basis wählen: „Zufluss (bezahlt)“ zählt nur tatsächlich bezahlte Rechnungen, „Rechnungsdatum“ zählt nach Rechnungsdatum. Stornierte Rechnungen zählen nie',
      'Verkaufte oder ausgemusterte Anlagen schreiben nur bis zum Abgangsmonat ab; im Abgangsjahr erscheint ihr „Restbuchwert abgegangener Anlagen“ als Ausgabe. Fehlt das Abgangsdatum, zeigt die Seite einen Hinweis und rechnet die Anlage nicht',
  ...
  '/dashboard/mitglieder': {
    ...
    schritte: [
      ...
      'Unter „💶 SEPA-Lastschrift erzeugen“ den „Fälligkeitstag (Ausführung)“ wählen und „⭱ SEPA-Datei erzeugen (…)“ klicken. Aufgenommen werden aktive Mitglieder und gekündigte, deren Vertrag am Fälligkeitstag noch läuft — jeweils mit Mandat, gültiger IBAN und Betrag über 0. In der Liste steht bei Gekündigten „zahlt bis …“',
  ...
  '/dashboard/foerder-angebot': {
    zweck: 'Ein förder-taugliches Angebot (Kostenvoranschlag) für einen Kunden erstellen: eigene Positionen oder eine Vorlage (Digitalisierung, IT-Sicherheit, Beratung), eine eigene Leistungsbeschreibung, eine angenommene Förderquote mit Live-Schätzung von Zuschuss und Eigenanteil, gespeichert und als PDF herunterladbar.',
    wer: 'chef',
    werText: 'Chef oder Mitarbeiter mit Freigabe (sensibel). 🗑 löscht ein gespeichertes Angebot nach einer Rückfrage.',
    schritte: [
      '„Kunde“ eintragen (vorhandene Kontakte werden beim Tippen vorgeschlagen) und den „Titel des Pakets“ prüfen',
      'Die eigenen Leistungen als Positionen eintragen oder unter „Vorlage laden“ ein Beispiel wählen und an Ihre Leistungen und Preise anpassen. „＋ Position hinzufügen“ fügt eine Zeile an, ✕ entfernt eine',
      'Optional die „Leistungsbeschreibung für den Förderantrag“ schreiben — sie steht so im PDF',
    ...
    probe: { anlegen: 'Ein Angebot für „Test Kunde“ mit einer der Vorlagen speichern.', loeschen: '🗑 beim Angebot, dann die Rückfrage bestätigen.' },
  ...
  '/dashboard/kfz/schaden': {
    ...
    schritte: [
      ...
      'Eingänge unter „Zahlungen": Datum, Betrag, „vom Versicherer"/„vom Kunden", „＋ Zahlung". Die Übersicht rechnet Rechnung, Eingegangen und Offen. Der Status folgt den Zahlungen: „Gekürzt" nur, wenn der Versicherer weniger als seinen Anteil gezahlt hat — eine offene Selbstbeteiligung des Kunden ist keine Kürzung. Eine falsch erfasste Zahlung entfernt das ✕ nach einer Rückfrage',
    ...
    probe: { anlegen: 'Einen Schadenfall mit Kennzeichen „TEST" über „＋ Schadenfall anlegen".', loeschen: 'Löschen gibt es hier nicht — den Fall auf „Abgeschlossen" setzen, dann verschwindet er bei „Nur offene Fälle". Eine gebuchte Zahlung entfernt das ✕ daneben nach einer Rückfrage.' },
  ...
  '/dashboard/objektzeiten': {
    ...
    schritte: [
      ...
      '„Auswertung je Objekt“: „🧾 Rechnung“ fragt zuerst den Empfänger ab (Kontakt wählen oder Namen eintragen), dann „Rechnung erstellen“. Auf die Rechnung kommen ALLE offenen, abrechenbaren Zeiten dieses Objekts (nicht nur des angezeigten Monats). Die Zeiten sind danach mit 🧾 als abgerechnet markiert — ein zweiter Klick rechnet sie nicht noch einmal ab',
      'Die Anschrift des Empfängers in der Rechnung prüfen, bevor sie verschickt wird',
  ...
  '/dashboard/bau-lv': {
    ...
    schritte: [
      ...
      '„🧾 Aus LV eine Rechnung erstellen“: Die Rechnung liegt danach unter „🧾 Rechnungen“. Gibt es schon eine, meldet die Seite „Zu diesem LV gibt es bereits eine Rechnung.“ Danach ist das LV gesperrt (🔒): Positionen lassen sich nicht mehr ändern oder löschen, Mehraufwand läuft über Nachträge mit eigener Rechnung. Wird die Rechnung storniert, ist das LV wieder frei',
      'Menge und „EP €“ müssen lesbare Zahlen sein (z. B. 12,5 und 48,90) — sonst meldet die Seite es, statt still 0 zu speichern',
