...
// ============================================================================
// ARGONAUT OS · components/DossierFreebie.tsx
//
// Das Freebie-Feld: E-Mail eintragen -> Bestaetigungslink -> Dossier als PDF.
//
// WARUM DIE DATEI UMGEZOGEN IST
// Sie lag bis zum 16.08. unter app/branchen/[slug]/. Dort war sie richtig,
// solange nur die Branchenseite sie brauchte. Seit auch die Vergleichsseite
// ein Freebie bekommt, waere sie an zwei Stellen — und zwei Fassungen desselben
// Formulars laufen frueher oder spaeter auseinander (die eine bekommt einen
// Rechtshinweis, die andere nicht). Der alte Pfad reicht diese Datei jetzt nur
// noch durch; jeder bestehende Import bleibt gueltig.
//
// OHNE BRANCHE FUNKTIONIERT ES AUCH
// `branche` und `slug` sind freiwillig. Fehlen sie, wird das allgemeine
// Dossier angefordert — dossierKey() macht daraus 'allgemein'. So kann
// dasselbe Formular auf der Vergleichsseite stehen, wo es keine Branche gibt.
//
...
export default function DossierFreebie({
  slug = '',
  branche = '',
  titel,
  text,
  ...
  slug?: string
  branche?: string
  /** Ueberschrift ueberschreiben. Ohne Angabe passend zur Branche. */
  titel?: string
  /** Beschreibungstext ueberschreiben. */
  text?: string
}) {
  ...
  const hatBranche = branche.trim().length > 0
  ...
  const absenden = async (e: React.FormEvent) => {
    ...
    try {
      const r = await fetch('/api/oeffentlich/dossier-optin', {
        ...
        body: JSON.stringify({ email, name: name || null, branche: slug || null }),
      })
    ...
    <div className="bg-[#0A1628] rounded-2xl p-10 mt-16 border border-[#C9A84C]/25">
      <div className="flex flex-col lg:flex-row lg:items-start gap-8">
        <div className="lg:w-1/2">
          ...
          <h3 className="text-white text-xl font-bold mb-2">
            {titel ?? (hatBranche ? `Das Branchen-Dossier ${branche}` : 'Das ARGONAUT-Dossier')}
          </h3>
          <p className="text-white/60 text-sm">
            {text ??
              (hatBranche
                ? `Wo in ${branche}-Betrieben die Zeit verloren geht, was das im Jahr kostet und welche Bausteine das abstellen — als PDF zum Mitnehmen. Kein Verkaufsgespräch, keine Kosten.`
                : 'Wo im Betriebsalltag die Zeit verloren geht, was das im Jahr kostet und welche Bausteine das abstellen — als PDF zum Mitnehmen. Kein Verkaufsgespräch, keine Kosten.')}
          </p>
        </div>
