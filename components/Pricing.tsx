...
const agenten = {
  start: [
    ...
    { code: "A3", name: "Der Waechter", beschreibung: "Ueberwacht alle laufenden Prozesse & meldet Abweichungen" },
    { code: "A4", name: "Der Koordinator", beschreibung: "Koordiniert agentenuebergreifende Aufgaben & Handoffs" },
    { code: "E1", name: "Der Akquisiteur", beschreibung: "Automatisiert Leadgenerierung und Erstansprache" },
    { code: "E2", name: "Der Verfolger", beschreibung: "Fuehrt automatisiertes Follow-up mit Interessenten durch" },
    { code: "E3", name: "Der Abschluss-Agent", beschreibung: "Unterstuetzt den Verkaufsprozess bis zum Abschluss" },
  ...
  pro: [
    ...
    { code: "B2", name: "Der Rechnungspruefer", beschreibung: "Prueft und verarbeitet eingehende Rechnungen" },
    { code: "B3", name: "Der Mahnungsagent", beschreibung: "Verwaltet automatisch Mahnlaeufe & Zahlungserinnerungen" },
    { code: "B4", name: "Der Berichter", beschreibung: "Erstellt Finanzberichte und KPI-Dashboards automatisch" },
    { code: "C1", name: "Der Sekretaer", beschreibung: "Verwaltet Termine, E-Mails und administrative Aufgaben" },
  ],
  business: [
    ...
    { code: "D1", name: "Der Content-Creator", beschreibung: "Erstellt Marketing-Content fuer alle Kanaele automatisch" },
    { code: "D2", name: "Der Social-Media-Agent", beschreibung: "Verwaltet und bespielt Social-Media-Kanaele automatisch" },
  ],
  enterprise: [
    { code: "C4", name: "Der Compliance-Waechter", beschreibung: "Ueberwacht Compliance-Anforderungen und Fristen" },
    { code: "D3", name: "Der SEO-Agent", beschreibung: "Optimiert Content und ueberwacht SEO-Performance" },
    { code: "D4", name: "Der Kampagnen-Manager", beschreibung: "Plant und steuert automatisierte Marketing-Kampagnen" },
    { code: "A10", name: "Der Sammler", beschreibung: "Aggregiert Daten aus allen Quellen fuer maximale OS-Intelligenz" },
  ],
};

const pakete = [
  {
    name: "START", agents: "8 Agenten", price: "1.500", total: "3.000", hot: false, badge: "",
    features: ["8 KI-Agenten aktiv", "Dashboard inklusive", "Priority Support", "1 Standort"],
    agentKeys: ["start"],
  },
  {
    name: "PRO", agents: "16 Agenten", price: "2.500", total: "4.000", hot: true, badge: "Beliebteste Wahl",
    features: ["16 KI-Agenten aktiv", "Dashboard inklusive", "Dedizierter Support", "1 Standort"],
    agentKeys: ["start", "pro"],
  },
  {
    name: "BUSINESS", agents: "20 Agenten", price: "4.500", total: "6.000", hot: false, badge: "",
    features: ["20 KI-Agenten aktiv", "Dashboard inklusive", "Account Manager", "1 Standort"],
    agentKeys: ["start", "pro", "business"],
  },
  {
    name: "ENTERPRISE", agents: "24 Agenten", price: "7.500", total: "9.000", hot: false, badge: "",
    features: ["24 KI-Agenten aktiv", "Dashboard inklusive", "VIP Support & Onboarding", "1 Standort"],
...
function PaketCard({ plan }: { plan: typeof pakete[0] }) {
  ...
  return (
    <div style={{ background: plan.hot ? "#0D1B3E" : "#ffffff", border: plan.hot ? "2px solid #C9A84C" : "1px solid #e5e5e5", borderRadius: "14px", padding: "20px 16px", display: "flex", flexDirection: "column", position: "relative" }}>
      ...
      <button onClick={() => setOpen(!open)} style={{ background: "transparent", border: plan.hot ? "1px solid rgba(201,168,76,0.4)" : "1px solid #e5e5e5", borderRadius: "6px", padding: "6px 10px", fontSize: "11px", color: plan.hot ? "rgba(255,255,255,0.6)" : "#666", cursor: "pointer", marginBottom: "10px", width: "100%", textAlign: "left" }}>
        {open ? "&#9650;" : "&#9660;"} KI-Agenten anzeigen ({alleAgenten.length})
      </button>
      {open && (
        <div style={{ marginBottom: "12px", maxHeight: "200px", overflowY: "auto" }}>
          {alleAgenten.map((a) => (
            <div key={a.code} style={{ padding: "5px 0", borderBottom: plan.hot ? "0.5px solid rgba(255,255,255,0.08)" : "0.5px solid #f0f0f0" }}>
              <p style={{ fontSize: "10px", fontWeight: 700, color: "#C9A84C", marginBottom: "1px" }}>{a.name}</p>
              <p style={{ fontSize: "10px", color: plan.hot ? "rgba(255,255,255,0.55)" : "#777" }}>{a.beschreibung}</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <div style={{ background: "#0D1B3E", borderRadius: "14px", padding: "24px 28px", marginBottom: "8px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "16px" }}>
            {[
              { title: "Vertrieb (10)", items: ["Lead-Aufnahme & Routing", "Angebot erstellen & senden", "Follow-up Tag 3 + Tag 7", "Terminvorschlag & Bestaetigung", "Lead-Qualifizierung KI", "Auftragsumwandlung", "Abschluss Won/Lost", "Interne Uebergabe", "Rueckrufwunsch erfassen", "Angebotsanfrage aufnehmen"] },
              { title: "Marketing (8)", items: ["Bewertungsanfrage nach Auftrag", "Reaktivierung inaktiver Leads", "Social Media Entwurf KI", "Leadquellen-Tagging", "Woechentlicher Performance-Report", "Content-Ideen KI", "Kampagnen-Textmodule", "Aktionsnachricht saisonal"] },
              { title: "Frontdesk (7)", items: ["Begruessung & Intent-Erkennung", "FAQ-Antworten automatisch", "Rueckrufwunsch erfassen", "Statusanfrage beantworten", "Dokumentenanfrage", "Eskalation an Mensch", "Abschlussfeedback anfragen"] },
            ].map((col) => (
              <div key={col.title}>
                <p style={{ fontSize: "10px", fontWeight: 700, color: "#C9A84C", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "6px" }}>{col.title}</p>
                {col.items.map((item) => (
                  <p key={item} style={{ fontSize: "10px", color: "rgba(255,255,255,0.6)", padding: "2px 0 2px 10px", position: "relative" }}>
          ...
          <p style={{ fontSize: "10px", color: "rgba(255,255,255,0.35)", marginTop: "14px", paddingTop: "12px", borderTop: "0.5px solid rgba(255,255,255,0.1)" }}>
            + Branchenspezifische Zusatz-Automatisierungen individuell erweiterbar &agrave; 85 &euro;/Monat
          </p>
        </div>
        ...
        <div style={{ borderTop: "1px dashed #ddd", paddingTop: "24px", marginBottom: "20px" }}>
          ...
          <div style={{ background: "#ffffff", border: "1.5px solid #e8dfc8", borderRadius: "12px", padding: "20px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", flexWrap: "wrap", marginBottom: "10px" }}>
            ...
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: "12px", color: "#aaa", display: "block" }}>/ Monat</span>
        ...
        <div style={{ background: "#f8f6f0", border: "1px solid #e8dfc8", borderRadius: "10px", padding: "14px 20px", textAlign: "center", marginBottom: "20px" }}>
          <p style={{ fontSize: "13px", color: "#0D1B3E" }}>
            <strong>Die 25 Standard-Automatisierungen</strong> decken die wichtigsten Prozesse ab &mdash; zus&auml;tzlich erhalten Sie branchenspezifische Automatisierungen, exakt auf Ihre Branche zugeschnitten.
          </p>
        </div>
