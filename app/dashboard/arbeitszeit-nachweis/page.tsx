  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.toggles}>
        <span style={{ fontSize: 'clamp(12px, 1.06vw, 17px)', color: C.textDim, textTransform: 'uppercase', letterSpacing: 1, marginRight: 4 }}>Betrieb:</span>
    ...
    <>
      <tr>
        <td style={styles.td}>
          ...
          <span style={{ color: C.textDim, marginLeft: 6, fontSize: 'clamp(12px, 1.06vw, 17px)' }}>{t.wochentag.slice(0, 2)}</span>
          {t.offen && <span style={{ color: C.green, fontSize: 'clamp(11px, 0.94vw, 15px)', marginLeft: 6 }}>● läuft</span>}
        </td>
          ...
          <td colSpan={5} style={{ ...styles.td, paddingTop: 0 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {t.verstoesse.map((v, i) => (
                <span key={i} style={{ fontSize: 'clamp(12px, 1.06vw, 17px)', color: ampelFarbe(v.schwere) }}>
                  ▸ {v.text}
                </span>
              ))}
            </div>
...
function Toggle({ an, setAn, label }: { an: boolean; setAn: (v: boolean) => void; label: string }) {
  return (
    <button onClick={() => setAn(!an)} style={{
      ...
      color: an ? C.text : C.textDim, fontSize: 'clamp(13px, 1.13vw, 18px)', cursor: 'pointer', fontFamily: 'inherit',
    }}>
...
const styles: Record<string, CSSProperties> = {
  ...
  input: { background: C.navy2, color: C.text, border: `1px solid ${C.border}`, borderRadius: 10, padding: '9px 12px', fontSize: 'clamp(14px, 1.25vw, 20px)', fontFamily: 'inherit', minWidth: 220 },
  ...
  pdfBtn: { background: C.gold, color: '#0A1628', border: 'none', borderRadius: 10, padding: '10px 18px', fontSize: 'clamp(14px, 1.25vw, 20px)', fontWeight: 800, fontFamily: 'inherit' },
  ...
  td: { padding: '9px 10px', fontSize: 'clamp(14px, 1.25vw, 20px)', borderBottom: '1px solid rgba(143,163,190,0.08)', verticalAlign: 'top' },
