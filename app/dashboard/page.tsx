      ...
      <main style={{ maxWidth: SHELL_MAX, margin: '0 auto', padding: `clamp(32px, 4vw, 56px) ${SHELL_PAD} 80px` }}>
        ...
        <section style={{ marginBottom: '32px' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '14px', marginBottom: '14px' }}>
            ...
            <span style={{ padding: '4px 14px', borderRadius: '999px', fontSize: 'clamp(12px, 1.06vw, 17px)', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: planColor, background: `${planColor}22`, border: `1px solid ${planColor}55` }}>{planLabel}</span>
            <span style={{ padding: '4px 14px', borderRadius: '999px', fontSize: 'clamp(12px, 1.06vw, 17px)', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: statusCfg.color, background: statusCfg.bg, border: `1px solid ${statusCfg.color}55`, display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <a href="/dashboard/start" style={{ textDecoration: 'none', display: 'block' }}>
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    <p style={{ margin: '0 0 2px', fontSize: 'clamp(15px, 1.31vw, 21px)', fontWeight: 800, color: '#C9A84C' }}>Einrichtung abschließen</p>
                    <p style={{ margin: 0, fontSize: 'clamp(13px, 1.13vw, 18px)', color: 'rgba(255,255,255,0.55)' }}>Onboarding & Zugangsdaten vervollständigen — dann ist Ihr System startklar.</p>
                  </div>
                </div>
                <div style={{ padding: '8px 20px', background: 'rgba(201,168,76,0.15)', border: '1px solid rgba(201,168,76,0.4)', color: '#C9A84C', borderRadius: '8px', fontWeight: 700, fontSize: 'clamp(13px, 1.13vw, 18px)', whiteSpace: 'nowrap' }}>Zur Einrichtung →</div>
              </div>
            </a>
          </section>
        )}
        ...
        <section style={{ marginBottom: '28px' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px', marginBottom: '14px' }}>
            ...
            <span style={{ fontSize: 'clamp(13px, 1.13vw, 18px)', color: 'rgba(255,255,255,0.4)' }}>Was Aufmerksamkeit braucht</span>
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
            {radar.length === 0 ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(76,175,125,0.12)', border: '1px solid rgba(76,175,125,0.4)', borderRadius: '12px', padding: '12px 18px', fontSize: 'clamp(14px, 1.25vw, 20px)', fontWeight: 600, color: '#4CAF7D' }}>
                <span>✅</span><span>Alles im grünen Bereich – nichts Dringendes.</span>
              </div>
            ) : radar.map((r, i) => (
              <a key={i} href={r.href} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: `${r.farbe}1e`, border: `1px solid ${r.farbe}55`, borderRadius: '12px', padding: '12px 18px', fontSize: 'clamp(14px, 1.25vw, 20px)', fontWeight: 600, color: '#fff', textDecoration: 'none' }}>
        ...
        <section style={{ marginBottom: '40px' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px', marginBottom: '18px' }}>
            ...
            <span style={{ fontSize: 'clamp(13px, 1.13vw, 18px)', color: 'rgba(255,255,255,0.4)' }}>Echtzeit-Kennzahlen</span>
          </div>

          {/* 💶 Finanzen */}
          <div style={{ marginBottom: '24px' }}>
            <div style={{ fontSize: 'clamp(12px, 1.06vw, 17px)', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: '12px', opacity: 0.85 }}>💶 Finanzen</div>
          ...
          {/* 🤝 Vertrieb */}
          <div style={{ marginBottom: '24px' }}>
            <div style={{ fontSize: 'clamp(12px, 1.06vw, 17px)', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: '12px', opacity: 0.85 }}>🤝 Vertrieb</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: '16px' }}>
          ...
          {/* 👥 Personal */}
          <div>
            <div style={{ fontSize: 'clamp(12px, 1.06vw, 17px)', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: '12px', opacity: 0.85 }}>👥 Personal</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: '16px' }}>
        ...
        <section style={{ marginBottom: '40px' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px', marginBottom: '18px' }}>
            ...
            <span style={{ fontSize: 'clamp(13px, 1.13vw, 18px)', color: 'rgba(255,255,255,0.4)' }}>{feed.length > 0 ? `${feed.length} Ereignisse` : 'Aktivitäts-Stream'}</span>
          </div>
          ...
          {feed.length === 0 ? (
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: '14px', padding: '40px 24px', textAlign: 'center' }}>
              <div style={{ fontSize: 'clamp(32px, 2.81vw, 45px)', marginBottom: '10px' }}>🌙</div>
              <p style={{ margin: 0, fontSize: 'clamp(15px, 1.31vw, 21px)', color: 'rgba(255,255,255,0.6)' }}>In den letzten 24 Stunden war es ruhig.</p>
              <p style={{ margin: '4px 0 0', fontSize: 'clamp(13px, 1.13vw, 18px)', color: 'rgba(255,255,255,0.35)' }}>Neue Leads, Aufträge, Rechnungen und Team-Aktivitäten erscheinen hier automatisch.</p>
            </div>
          ) : (
              ...
              {feed.map((e, i) => (
                <a key={i} href={e.href} style={{
                  ...
                  <span style={{
                    ...
                    display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(16px, 1.38vw, 22px)',
                  }}>{e.icon}</span>
                  <span style={{ flex: 1, minWidth: 0, fontSize: 'clamp(14px, 1.25vw, 20px)', color: 'rgba(255,255,255,0.85)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{e.text}</span>
                  <span style={{ flexShrink: 0, fontSize: 'clamp(12px, 1.06vw, 17px)', color: 'rgba(255,255,255,0.4)' }}>{vorZeit(e.zeit)}</span>
                </a>
              ))}
            </div>
          )}
          ...
          {kacheln.map((k) => (
            <a key={k.href} href={k.href} style={{
              ...
              <div style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(22px, 1.94vw, 31px)',
              }}>{k.icon}</div>
              <p style={{ margin: '0 0 4px', fontSize: 'clamp(17px, 1.5vw, 24px)', fontWeight: 800, color: '#FFFFFF' }}>{k.titel}</p>
              <p style={{ margin: 0, fontSize: 'clamp(13.5px, 1.19vw, 19px)', color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>{k.text}</p>
            </a>
          ))}
        </div>
      </section>
        ...
        <a href="/dashboard/mein-bereich" style={{ textDecoration: 'none', display: 'block' }}>
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
              <span style={{ fontSize: 'clamp(28px, 2.44vw, 39px)' }}>🙋</span>
              <div>
                <p style={{ margin: '0 0 4px', fontSize: 'clamp(15px, 1.31vw, 21px)', fontWeight: 700, color: '#FFFFFF' }}>Zu meinem Bereich</p>
                <p style={{ margin: 0, fontSize: 'clamp(13px, 1.13vw, 18px)', color: 'rgba(255,255,255,0.5)' }}>Resturlaub, Schichten und Ihre Arbeitszeit auf einen Blick.</p>
              </div>
            </div>
            <div style={{ padding: '8px 20px', background: 'rgba(0,229,255,0.12)', border: '1px solid rgba(0,229,255,0.3)', color: '#00e5ff', borderRadius: '8px', fontWeight: 700, fontSize: 'clamp(13px, 1.13vw, 18px)', whiteSpace: 'nowrap' }}>Öffnen →</div>
          </div>
        </a>
      </section>
    </main>
