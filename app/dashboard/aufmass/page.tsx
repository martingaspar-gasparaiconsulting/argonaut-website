...
// Aufmaße erfassen: Kopf (Kunde/Projekt/Ort) + Messpositionen (Menge/Einheit/
// Preis, inline-editierbar) + Live-Summe (Mengen je Einheit + Netto). KiAuge.
// Generisch: Bau, GaLaBau, Maler, Forst. Bestätigung vor Kopf-Schreiben,
// Positionen sofort gespeichert. Design 1:1 wie das übrige Dashboard.
...
type AufmassRow = {
  ...
  aufmass_datum: string; bearbeiter: string | null; notiz: string | null; archiviert: boolean;
};
...
export default function AufmassPage() {
  ...
  const laden_ = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.from('aufmasse').select('*')
        .eq('owner_user_id', uid).eq('archiviert', false)
        .order('aufmass_datum', { ascending: false });
      if (error) throw error;
      setAufmasse((data as AufmassRow[]) ?? []);
    } catch (e: unknown) {
  ...
  function neu() { setForm(LEER); setPositionen([]); setGespeichertHinweis(false); setModalAuf(true); }
  async function bearbeiten(a: AufmassRow) {
    ...
    setGespeichertHinweis(false); setModalAuf(true);
  ...
  async function archivieren(a: AufmassRow) {
    ...
    try {
      const { error } = await supabase.from('aufmasse').update({ archiviert: true, aktualisiert_am: new Date().toISOString() }).eq('id', a.id);
      if (error) throw error;
  ...
  async function positionHinzufuegen() {
    if (!uid || !form.id) { setFehler('Bitte zuerst das Aufmaß speichern (Anlegen), dann Positionen hinzufügen.'); return; }
    try {
      const { error } = await supabase.from('aufmass_positionen').insert({
        owner_user_id: uid, aufmass_id: form.id, bezeichnung: '', menge: 0, einheit: 'm²',
  ...
  async function positionAendern(id: string, patch: Partial<PositionRow>) {
    ...
    try {
      await supabase.from('aufmass_positionen').update({ ...patch, aktualisiert_am: new Date().toISOString() }).eq('id', id);
    } catch { /* still */ }
  }
    ...
    <div style={styles.page}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={styles.sub}>Mengen vor Ort erfassen — Flächen, Längen, Stück. Grundlage für die Abrechnung.</p>
        </div>
            ...
            {aufmasse.map((a) => {
              ...
              return (
                <button key={a.id} onClick={() => bearbeiten(a)} style={styles.listItem}>
                  ...
                  <span style={{ fontSize: 12, color: sd.farbe, flexShrink: 0 }}>{sd.label}</span>
                </button>
              );
            })}
          </div>
            ...
            {!form.id ? (
              ...
              <div style={styles.sektion}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  ...
                  <button onClick={positionHinzufuegen} style={styles.miniBtn}>+ Position</button>
                </div>

                {positionen.length === 0 ? (
                  <div style={{ color: C.textDim, fontSize: 13 }}>Noch keine Positionen. Klick „+ Position".</div>
                ) : (
                    ...
                    <table style={styles.posTable}>
                      <thead>
                        <tr>
                          ...
                          <th style={{ ...styles.posTh, width: 90 }}>Menge</th>
                          <th style={{ ...styles.posTh, width: 90 }}>Einheit</th>
                          <th style={{ ...styles.posTh, width: 100, textAlign: 'right' }}>Einzelpreis</th>
                          ...
                          return (
                            <tr key={p.id}>
                              ...
                              <td style={styles.posTd}>
                                <input style={styles.posInput} defaultValue={p.bezeichnung ?? ''} onBlur={(e) => positionAendern(p.id, { bezeichnung: e.target.value })} placeholder="z. B. Pflaster verlegen" />
                              </td>
                              <td style={styles.posTd}>
                                <input style={{ ...styles.posInput, textAlign: 'right' }} defaultValue={String(p.menge ?? 0)} onBlur={(e) => positionAendern(p.id, { menge: num(e.target.value) ?? 0 })} />
                              </td>
                              <td style={styles.posTd}>
                                <select style={styles.posInput} value={p.einheit ?? 'm²'} onChange={(e) => positionAendern(p.id, { einheit: e.target.value })}>
                ...
                <div style={styles.summeZeile}>
                  <span style={{ color: C.textDim, fontSize: 13 }}>
                    {summe.mengenJeEinheit.length > 0
...
const styles: Record<string, CSSProperties> = {
  ...
  posTable: { width: '100%', borderCollapse: 'collapse', minWidth: 620 },
