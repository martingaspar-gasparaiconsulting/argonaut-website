...
// ============================================================
// ARGONAUT OS · Öffentliche Anmeldeseite (Double-Opt-In, ohne Login)
// /anmelden/<slug> — Interessent trägt sich ein, bekommt danach eine
// Bestätigungsmail (DOI). Liest & schreibt nur über /api/oeffentlich/optin.
// Branding DES BETRIEBS (Akzentfarbe aus dem Profil).
// ============================================================
...
export default function AnmeldenSeite() {
  ...
  const [akzent, setAkzent] = useState('#C9A84C');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setFehler(null);
      try {
        const res = await fetch(`/api/oeffentlich/optin?slug=${encodeURIComponent(slug)}`);
        const j = await res.json();
        if (!res.ok) {
          setFehler(j?.error || 'Anmeldeseite nicht verfügbar.');
        } else {
          setBetrieb(j.betrieb || '');
          setTitel(j.titel || 'Newsletter abonnieren');
          setText(j.text || '');
  ...
  async function absenden() {
    ...
    setSenden(true);
    try {
      const res = await fetch('/api/oeffentlich/optin', {
        method: 'POST',
      ...
      const j = await res.json();
      if (!res.ok || !j?.ok) {
        setFehler(j?.error || 'Anmeldung fehlgeschlagen.');
      } else {
        setFertig(j.status === 'bereits' ? 'bereits' : 'bestaetigung');
      }
    } catch {
  ...
  return (
    <main style={S.page}>
      <div style={S.wrap}>
        {laden ? (
          ...
          <div style={S.card}>
            ...
            <label style={S.consent}>
              ...
              <span>Ja, ich möchte E-Mails von {betrieb} erhalten. Ich kann mich jederzeit über den Abmelde-Link in jeder E-Mail wieder abmelden.</span>
            </label>
        ...
        <div style={S.footer}>Anmeldung bereitgestellt über ARGONAUT OS · Double-Opt-In</div>
      </div>
    </main>
  );
}
