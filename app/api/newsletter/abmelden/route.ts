...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (error) return seite('Newsletter', '#1a2332', 'Abmeldung', 'Es gab ein technisches Problem. Bitte versuchen Sie es später erneut.');
    if (!data || data.length === 0) {
      return seite('Newsletter', '#1a2332', 'Abmeldung', 'Dieser Link ist nicht mehr gültig — vielleicht sind Sie bereits abgemeldet.');
    }
    ...
    return seite(
      ...
      `Sie wurden vom Newsletter von ${firma} abgemeldet und erhalten keine weiteren E-Mails mehr. Sie können dieses Fenster schließen.`,
    );
  } catch {
    return seite('Newsletter', '#1a2332', 'Abmeldung', 'Es gab ein technisches Problem. Bitte versuchen Sie es später erneut.');
  }
}
