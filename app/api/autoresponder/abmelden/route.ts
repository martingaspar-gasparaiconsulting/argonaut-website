...
//
// OEFFENTLICH (kein Login). Wird ueber den Abmelde-Link in jeder Autoresponder-
// Mail aufgerufen: GET ?token=<abmelde_token>. Setzt den Lauf auf
// status='abgemeldet' (Service-Role, umgeht RLS) -> es folgen keine weiteren
// Schritte. Zeigt eine schlichte Bestaetigungsseite im Branding DES KUNDEN.
// §7 UWG: Abmeldung muss ohne Huerde gehen.
// ============================================================================
...
export async function GET(req: Request) {
  ...
  if (!token) return seite('Info-Serie', '#1a2332', 'Abmeldung', 'Kein gültiger Abmelde-Link.');
  ...
  try {
    ...
    if (ownerId) {
      ...
      firma = (pp.firma_name || '').trim() || 'Info-Serie';
