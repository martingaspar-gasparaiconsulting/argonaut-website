...
export default function BranchenPageClient({ slug }: { slug: string }) {
  ...
  const agentenVerfuegbar = agentenAll.filter(a =>
    (PAKET_ORDER[AGENT_META[a]?.minPaket || 'PRO'] ?? 3) <= PAKET_ORDER[selectedPaket]
  )
      ...
      <section className="bg-[#0A1628] pt-40 pb-16 px-6">
        <div className="max-w-5xl mx-auto">
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {PAKETE.map(p => (
                <button key={p.key} onClick={() => setSelectedPaket(p.key)}
                  style={{
                    padding: '8px 16px', borderRadius: '999px', border: `1px solid ${selectedPaket === p.key ? p.color : 'rgba(255,255,255,0.15)'}`,
          ...
          <div className="flex flex-wrap gap-12">
            <div>
              <div className="text-[#C9A84C] text-2xl font-bold">−{Math.round(stundenPaket * 100 / 40)} %</div>
            ...
            <div>
              <div className="text-[#C9A84C] text-2xl font-bold">{agentenVerfuegbar.length} Agenten</div>
              <div className="text-white/40 text-xs mt-1">im {paket.label}-Paket aktiv</div>
            </div>
          </div>
        </div>
      </section>
            ...
            {agentenAll.map((agentName, i) => {
              ...
              return (
                <div key={agentName} style={{
                  ...
                  gap: '16px',
                  position: 'relative',
                  opacity: isVerfuegbar ? 1 : 0.5,
                }}>
                  {isTop && (
                    <div style={{ position: 'absolute', top: '12px', right: '16px', fontSize: '10px', fontWeight: 700, color: '#C9A84C', background: 'rgba(201,168,76,0.15)', border: '1px solid rgba(201,168,76,0.3)', borderRadius: '999px', padding: '2px 8px' }}>
                      TOP
                    </div>
                  )}
                  {!isVerfuegbar && (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
                      ...
                      {isVerfuegbar && (
                        <span style={{ fontSize: '10px', fontWeight: 700, color: isTop ? '#0A1628' : meta?.paketColor || '#C9A84C', background: isTop ? meta?.paketColor || '#C9A84C' : `${meta?.paketColor || '#C9A84C'}22`, border: `1px solid ${meta?.paketColor || '#C9A84C'}55`, borderRadius: '999px', padding: '2px 8px' }}>
                  ...
                  <div style={{ fontSize: '11px', fontWeight: 700, color: isTop ? 'rgba(255,255,255,0.3)' : '#d1d5db', flexShrink: 0, paddingTop: '2px' }}>
                    #{i + 1}
                  </div>
                </div>
              )
